function reconImage = katsevichRecon(reconImage, projectionData, scanGeometry, configs)


ProjNumber = projectionData.getProjectionNum();
ScanR = scanGeometry.getScanRadius();
StdDis = scanGeometry.getSourceToDetectorDistance();
ProjScale = scanGeomtry.getNumberOfProjectionsPerTurn();
HelicP= scanGeometry.getHelicalPitch();

ProjCenter = (ProjNumber+1) / 2; % This is not touched yet.
FilterCoeFF = 0.5; % This is not touched yet

YL = projectionData.getDetectorResoU();
ZL = projectionData.getDetectorResoV();

RecSX = reconImage.getReconResoX();
RecSY = reconImage.getReconResoY();
RecSZ = reconImage.getReconResoZ();
...
% Derivation Mode is not set as the parameter yet
DerMode       = 1; % "1"  chain rule 
                   % "0"  direct  method
                   
ObjR  = reconImage.getObjRadius();
...
% TODO: We only support the panel detector now.
...
% 
% 
% function RecMatrix=FunctionKat_v6(COE,FilteringMode,Phantom, keepIntermediateResults)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
