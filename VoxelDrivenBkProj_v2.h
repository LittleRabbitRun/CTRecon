...
void WORKERFUNCTION(DATATYPE *data)
{
    ...
    TYPE HalfY, HalfZ, DeltaX, DeltaY, DeltaZ;
	...
	RecMY    = int(geom[2][2]);
	RecMZ    = int(geom[2][3]);	
    DeltaX   = TYPE(geom[2][4]);
    DeltaY   = TYPE(geom[2][5]);
    DeltaZ   = TYPE(geom[2][6]);
    ...
    TYPE DeltaL, DeltaU, DeltaV;
	TYPE XCenter, YCenter, ZCenter;
...
// 	DeltaZ = 2*ObjR/RecMZ;
	...
	for(loop=0;loop<RecMX;loop++)
		xCor[loop] = (loop-XCenter)*DeltaX; // All voxel x positions
	for(loop=0;loop<RecMY;loop++)
		yCor[loop] = -(loop-YCenter)*DeltaY; // All voxel y positions
	for(loop=0;loop<RecMZ;loop++)
		zCor[loop] = (loop-ZCenter)*DeltaZ; // All voxel z positions
		        ...
		        for (Zindex=0; Zindex<RecMZ; Zindex++)	
				{
					...
					if(!((BAngle>=ProjNum-1)||(TAngle<=0)))
					{
						...
						for(ProjIndex=LoopBegin;ProjIndex<=LoopEnd;ProjIndex++)
						{
							...
							temp = bilinearInterpolation(YY, ZZ, YLZL, YL, ProjIndex, Proj) * Coef[ProjIndex];
							tpdata += temp / factor;
						}
